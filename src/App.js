...
function App() {
  ...
  const fetchVideos = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.from('videos').select('*').order('created_at', { ascending: false }).limit(10);
  ...
  const handleViewChange = useCallback((newView) => updateState({ view: newView }), [updateState]);
  const handleFloatingActionClick = useCallback(() => handleViewChange('upload'), [handleViewChange]);

  const displayedVideos = useMemo(() => videos.slice(0, visibleVideos), [videos, visibleVideos]);
  const showUploadPage = useMemo(() => view === 'upload' && user, [view, user]);
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-black relative safe-top safe-bottom">
      <NavBar user={user} isAdmin={isAdmin} view={view} onViewChange={handleViewChange} onLogout={handleLogout} onAuthClick={() => updateState({ authView: 'login' })} />
      {isLoading && videos.length === 0 && <LoadingSpinner />}
      
      {showUploadPage ? (
        <UploadPage onUploadSuccess={handleAuthSuccess} user={user} isAdmin={isAdmin} />
      ) : showRequireAuth ? (
        ...
        <div className="h-full w-full overflow-y-scroll snap-y hide-scrollbar" onScroll={handleScroll} style={{ scrollBehavior: 'smooth' }}>
          {showEmptyState ? <EmptyState user={user} /> : displayedVideos.map((video, index) => (
            <VideoCard key={video.id} video={video} isActive={index === activeVideoIndex} user={user} isAdmin={isAdmin} />
          ))}
        </div>
      )}
      
      {view === 'feed' && user && <FloatingActionButton onClick={handleFloatingActionClick} />}
      {view === 'feed' && <BottomNav user={user} onLogout={handleLogout} onAuthClick={() => updateState({ authView: 'login' })} />}
    </div>
  );
}
